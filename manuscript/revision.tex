...
sect. \emph{Diedropetala} \textcolor{red}{Huth. After originating in the Himalaya-Hengduan region of Asia \textasciitilde~32 Ma, \emph{Delphinium} L. diversified into 650-700 species,
dispersing and speciating west through Eurasia, the Mediterranean, and into Africa, and east through eastern Asia, 
only recently
arriving in western North America \textasciitilde~3 Ma, where it rapidly radiated 
into approximately 60 species} \citep{jabbour_phylogeny_2012}.
% 
...
flow between species remains unknown. \textcolor{red}{Our primary objectives in this 
study are to use genome-wide data to resolve the phylogenetic backbone of 
\emph{Delphinium} sect. \emph{Diedropetala} and investigate the frequency
of introgressive hybridization with respect to species barriers and geography.
% extent to which widespread hybridization and 
% introgession has contributed to its diversification throughout North America. 
To achieve this,} we generated a new reduced-representation genomic dataset to 
...
taxa collected from multiple geographically disjunct populations. \textcolor{red}{When possible, we collected species from their type specimen localities, and our sampling includes
at least one taxon as a representative of each subsection in the FNA treatment of
\emph{Delphinium}, including the type species for subsections \emph{Bicoloria} (Rydberg) N. I. Malyutin, \emph{Exaltata} 
...
lineage sorting, 
\textcolor{red}{
we also employed coalescent-based species tree inference approaches.
...
ASTRAL IV v1.23.4.6 \citep{zhang_aster_2025}. 
...
\textcolor{red}{
Among our two analyses that grouped individuals into 41 populations, or 
subsampled a single individual from each of those populations, the resulting
topologies were similar. The SNP-based caster population analysis recovered the 
same relationships as in the caster analysis of all individuals. The ASTRAL species
tree analysis based on inferred gene trees also recovered a similar topology, but 
with many internal edges receiving <50\% support (Fig.~\ref{fig:S4}). 
This analysis was based on
3,121 gene trees that passed our filtering thresholds, but the information content
in individual gene trees was very low. The average number of internal edges per tree
was only 2.18 after collapsing edges with <50\% support. Therefore, we put greater 
confidence in the raxml-ng and caster results. 
}
...
% 
\textcolor{red}{They all exhibit moderate to tall stems (35-150 cm) and lobed leaves (2-9 cm x 3-15 cm), with racemes composed of 6-50 flowers that are purple, blue, or pale-blue with medium length (9-18 mm) spurs.} 
...
Members of this clade vary in stature (the subarctic \emph{D. brachycentrum} is short \textcolor{red}{(20-50 cm) compared to \emph{D. glaucum} and \emph{D. exaltatum} (70-200 cm)}) 
but are united by having glabrous to glaucous stems, moderate‑length spurs \textcolor{red}{(9-17 mm)}, 
...
\textcolor{red}{(Fig.~\ref{fig:S5},~\ref{fig:S6}).}
% 
...
in Fig.~\ref{fig:fbranch} is likely to be true. \textcolor{red}{Both analytical and bioinformatic
errors can inflate these estimates, and extinct and unsampled lineages may also contribute to these patterns. We suspect that some of the hypothetical introgression events may
represent false positives that can arise from non-independent signals of
admixture among multiple hybridizing taxa \citep{eaton_historical_2015, malinsky_dsuite_2021}.} Because each test examines only one quartet of taxa at a time, and different sets of taxa may share different sets of loci in RAD-seq datasets, even methods like the f-branch statistic that aim to account for the non-independence among tests may yield false positives. Nevertheless, ABBA-BABA tests have been widely applied to RAD-seq datasets \citep{heliconius_genome_consortium_butterfly_2012, eaton_inferring_2013, eaton_historical_2015, mccluskey_phylogeny_2015, vargas_conflicting_2017},
...
generally contain sufficient information to infer reliable gene trees to use as input to common phylogenetic network inference tools \citep{solis-lemus_inferring_2016}.
% 
Thus, we were not able to confidently apply this approach to our data.
\textcolor{red}{The limitations of RAD-seq loci also inhibited us from inferring a reliable chloroplast phylogeny as a point of comparison and validation with our nuclear phylogenies.
% 
...
\begin{figure}[p]
	...
	\caption{
		Astral species tree inferred from 3,121 gene trees from individual RAD-seq loci.
	}
	\label{fig:S4}
\end{figure}
...
\begin{figure}[p]
	\centering
	% \includegraphics[width=0.95\textwidth]{./figures/FigS6.pdf}
	\caption{
