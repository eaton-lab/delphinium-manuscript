...
into approximately 60 species since its arrival to western North America an 
...
the FNA treatment.
Other notable regional treatments of \emph{Delphinium} include:
...
In addition, two molecular phylogenetic studies have used
...
species tree relationships from among discordanct (conflicting) phylogenetic signals, 
...
To achieve a "population-level" phylogeny we additionally ran 'caster-site' using the same settings as above but with
...
observed within \emph{D. distichum}, \emph{D. nuttallianum}, \emph{D. ramosum}, and 
\emph{D. alpestre} (addressed below in the context of admixture).
% 
...
\begin{figure}[t!]
	\centering
	\includegraphics[width=0.95\textwidth]{./figures/Fig2_sptree_newcolor.pdf}
	\caption{
		...
		(Photo credits: \emph{D. viridescens} from the Burke Herbarium at the University of Washington, and \emph{D. xantholeucum} from Eastern Washington University).
	}
	\label{fig:sptree}
\end{figure}
...
A set of higher-level clades was recovered across all trees that show a strong
concordance with biogeographic regions, which we designated accordingly: GB1 and GB2 (Great Basin), PNW (Pacific Northwest), CAL (California), IMW (Intermountain West), ALCAN (Alaska/Canada), ENA (Eastern North America), and SW (Southwestern US) (Fig.~\ref{fig:sptree}a-b). 
% 
None of these
...
% 
The grouping of clades IMW, ALCAN, ENA, and SW is consistently recovered with 
...
Basin, \emph{D. stachydeum}, from northern Nevada, which is the sole representative of its clade, designated GB2. 
% 
...
% 
They all share a medium stature with moderate sized inflorescences of blue-purple flowers with medium length (9-18 mm) spurs. 

% Jared add info about lack of eastern species samples; missing clade here essentially 
The ALCAN clade corresponds primarily to \emph{D. brachycentrum}, which is endemic to 
...
America and exhibits a disjunct pattern between the Appalachian and Ozark Mountains \citep{mohn2021exaltatum}. The two populations of \emph{D. glaucum} group monophyletically in the caster topologies, 
...
The ENA clade is represented by a single species, \emph{D. carolinianum}, whose range begins east of the Rocky Mountains. Inclusion of the remaining \emph{Delphinium} taxa east of the Rockies is a vital next step that will help complete the phylogeny of \emph{Delphinium} sect. \emph{Diedropetala} and likely clarify the proper relationship between \emph{D. exaltatum} and the other ALCAN species.
...
\begin{figure}[t!]
	...
	\caption{
        ...
        % 
        Columns (x-axis) correspond to donor lineages, and rows (y-axis) to putative introgressive recipient lineages. Dotted lines on the y-axis indicate internal branches of the tree, with all taxa below a dotted line belonging to that respective branch. Each cell shows the excess allele sharing between the donor and the descendants of one of the recipient lineages relative to its sister lineage.
        %
...
reported observations that both species can hybridize with \emph{D. nudicaule} \citep{warnock_taxonomic_1995}.
% 
...
\subsubsection{Taxonomic clarification in light of admixture}
% 
These admixture results help to resolve long-standing disagreements 
...
\emph{D. barbeyi}. 
% 
 Our phylogenetic results show \emph{D. occidentale} as distinct from both \emph{D. barbeyi} and \emph{D. exaltatum}, and most closely related to \emph{D. glaucescens} with 100\% bootstrap support. In addition, our admixture results show that while \emph{D. occidentale} is an introgressive recipient of both \emph{D. glaucum} 
and \emph{D. barbeyi}, it also appears to be an introgressive donor to multiple taxa within the PNW clade. Given that nearly all taxa in our study show some evidence of admixture, we propose that \emph{D. occidentale} is just as deserving as any other taxon of species status. Our replicate population sampling, multiple phylogenomic analyses, and admixture 
...
\begin{figure}[t!]
	\centering
	  \includegraphics[width=0.99\textwidth]{./figures/ramosum-robustum-alpestre4}	
	\caption{
...
We refer to each of these populations exhibiting the distinct high elevation diminutive phenotype as \emph{D. cf. alpestre}.
...
% 
Using a more densely subsampled sequence alignment (24 samples x 780,187 bp with 22.5\% missing data) generated for this clade while using the CAL clade as a closer outgroup, we inferred an additional ML concatenation tree which further confirmed the phylogenetic relationships observed in the larger tree (Fig.~\ref{fig:robustum}a).
% 
As with the broader phylogenetic patterns in \emph{Delphinium} sect. \emph{Diedropetala}, relationships among populations within this clade match closely with geography, grouping into a northern, southeastern, and southwestern clade. 
%  
...
Further work is needed to investigate the role of introgression, plasticity, 
or the replicated adaptation to an alpine environment in driving this pattern.
...
% 
Following the Phylocode protocol \citep{deQueiroz2020phylo}, we propose a set of formal taxonomic names for the higher-level clades consistently recovered across our analyses, along with additional supporting evidence (Appendix 1). For this analysis, we employed an agent-based ChatGPT (OpenAI, 2025) to curate morphological and ecological data for \emph{Delphinium} species from the Flora of North America (FNA) and other authoritative referenced sources. The agent performed structured searches for each taxon, extracting details such as stem height, leaf morphology, inflorescence size, sepal colour, spur length and habitat from the FNA descriptions and field guides. After collating these character states, the agent grouped taxa into clades based on our stated phylogenetic hypotheses and identified synapomorphies that distinguish each clade. It then drafted clade definitions following the PhyloCode’s minimum‑clade format, proposed etymologically appropriate names, and highlighted diagnostic characters, exclusions and comparisons with other clades. Citations were numbered and hyperlinked to the original sources. The process was iterative with feedback from the authors to revise formatting and suggest additional comparisons. These formal clade descriptions will 
...
many of these hypotheses by investigating genomic admixture as an evolutionary
consequence of hybrid introgression.
% 
...
This may indicate that some hybrids are infertile, or it may reflect the fact
that introgression is sometimes geographically localized, and 
occurred outside of the range of populations sampled in this study.
% 
...
more reference genomes to improve paralogy estimation; and employing more advanced
...
The term syngameon is used to describe a set of species that are connected
by hybridization. Perhaps the most famous example involves oak trees, 
where patterns of interspecific hybridization have been intensely studied
[Hardin 1975], and more recently, patterns of introgression validated 
through comparative genomic methods \citep{ribicoff_introgression_nodate}. 
% 
A notable feature of syngameons is that not all species hybridize evenly, 
yet through a network of interconnected hybridizing species -- with a few
species functioning as super-hybridizing nodes -- it is possible for genes
to be exchanged, at least in theory, between any two species in the complex
[boecklen].
% 
This is precisely the pattern that we have observed in North American 
\emph{Delphinium}. 
...
...
% \emph{D. californicum} can be easily confused with \emph{D. glaucum} or \emph{D. exaltatum}, and is most readily distinguished by its western distribution and earlier flowering time \citep{Warnock_1997_FNA_Delphinium}. It is, however, quite distantly related in our results.
...
[NOTE: maybe drop or shorten these paragraphs when finishing the syngameon section]
The massive extent of introgression that appears to have occurred throughout the history
of North American \emph{Delphinium} raises the question of whether our phylogenetic results represent historical patterns of populations splitting across geographic or ecological barriers, versus more recent patterns of gene flow that may overwrite such historical patterns.
% 
In other words, do the geographically clustered clades recovered in our phylogeny represent diversification \emph{in situ} within each of these regions, or does the genomic similarity of species within the same region arise after they disperse into sympatry, as a consequence of introgression? 
% 
This distinction is important for understanding not only the diversification history of \emph{Delphinium}, but also for using \emph{Delphinium} to understand the broader biogeographic history of western North America. 
% 
Our results support X not Y. 
We believe that we can rule out the second hypothesis. 
% 
Should we believe that California, rather than Alaska and PNW, is the earliest diverging lineage?
...
% 
Overall, we discover an overwhelming signal of geographically structured ancestry, 
...
All sequence data are available on the NCBI sequence read archive under project
identifier PRNJXXX. Phylip formatted sequence alignment files and newick formatted tree files are available on DRYAD at doi/.... The newick file of our species tree hypothesis inferred by caster-site with population mapping for 41 taxa was submitted to Treebase (...).
