...
that has shaped the diversification of many endemism plant lineages.
% fostered
...
impede initiatives to quantify biodiversity and track its decline. 
...
\cite{rydberg_delphinium_1899, ewan_genus_1942, wilde1931studies, cronquist_intermountain_1984, ackerfield_flora_2022, chambers_comments_2018, lewis_taxonomic_1954,koontz_delphinium_2012,WeberWittmann2012}.
...
\begin{figure}[t!]
	...
	\caption{
        Heatmap of f-branch ($f_b$) statistics summarizing admixture on the reference species tree of \emph{Delphinium} section \emph{Diedropetala}, with
        colored blocks indicating the major clades described above.
        % 
        Rows correspond to putative introgressive recipient lineages, and columns to donor lineages. Each cell shows the excess allele sharing between the donor  and the descendants of one of the recipient lineages relative to its sister lineage.
        %
        ...
        %
        Arrows are drawn between branches on the left tree as a hypothesized summary of the strongest admixture signals, with arrow colors representing their mean $f_b$ value. 
	}
	\label{fig:fbranch}
\end{figure}
...
\begin{figure}[t!]
	...
	\caption{
        ...
        (c) A geomic PCA similarly groups samples by geography. Low opacity points show variation over replicate runs using randomly sampled subsets of
...
% 
There is substantial disagreement across floras concerning the distinction between \emph{D. ramosum}, \emph{D. robustum}, and \emph{D. alpestre} 
...
The final \emph{D. ramosum}-like population, from the southwestern San Juan Mountains, 
...
% 
As with the broader phylogenetic patterns in \emph{Delphinium} section \emph{Diedropetala}, relationships among populations within this clade match closely with geography, grouping into a northern, southeastern, and southwestern clade. 
%  
...
This also grouped samples into three distinct clusters corresponding to the same clades recovered in our phylogenetic analyses (Fig.~\ref{fig:robustum}c)
% 
...
Here, the southeastern and northern clades from this complex both appear intermediate between a cluster formed by all other members of the SW clade, and a cluster representing \emph{D. geyeri} from the IMW clade.
% 
...
and replicated adaptation in driving this pattern.
...
TO ADD: propose new clade names using PhyloCode conventions;

We summarized the taxonomic descriptions of taxa in each clade to determine traits that unite our newly circumscribed clades. Intermontanus united by trait, trait, and trait; Californicae...
...
ADD sentence about exaltatum study to reference that paper.
